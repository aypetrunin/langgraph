...
def _extract_usage(ai_msg: AIMessage) -> Dict[str, int]:
    ...
    # details
    prompt_details = (
        tu.get("prompt_tokens_details", {}) or um.get("input_token_details", {}) or {}
    )
    completion_details = (
        tu.get("completion_tokens_details", {})
        or um.get("output_token_details", {})
        or {}
    )

    usage["prompt_cached"] = int(
        (prompt_details.get("cached_tokens") or prompt_details.get("cache_read") or 0)
        or 0
    )
    usage["completion_reasoning"] = int(
        (
