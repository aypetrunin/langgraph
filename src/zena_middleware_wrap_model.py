...
from .mcp.registry import resolve_backend_name, MCP_BACKEND_ALENA_5020, MCP_BACKEND_LIST_5007, MCP_BACKEND_CLASSIC
...
class DynamicSystemPrompt(AgentMiddleware):
    async def awrap_model_call(
        ...
        # Можно заменить DebugUndefined на StrictUndefined, если хочешь прям fail-fast.
        jinja = Environment(undefined=DebugUndefined)
        system_prompt = jinja.from_string(source).render(**data)

        # Сохраняем отрендеренный prompt
    ...
    async def _load_template_source(self, request: ModelRequest, data: dict, is_dev: bool) -> str:
        ...
        - Если есть URL (контекст или data) — читаем Google Doc
          (в dev можно брать URL и из runtime.context["_prompt_google_url"])
...
class ToolSelectorMiddleware(AgentMiddleware):
    ...
    Также выбирает модель в зависимости от:
    - mcp_port
    - dialog_state
    """
    ...
    STAGE_TOOLS_CLASSIC: dict[str, set[str]] = {
        ...
        "selecting": {"zena_product_search", "zena_remember_product_id"},
        "remember": {"zena_avaliable_time_for_master"},
        "available_time": {"zena_record_time", "zena_avaliable_time_for_master"},
    ...
    async def _select_model(self, state: dict):
        ...
        if backend == MCP_BACKEND_LIST_5007:
            return models.model_4o if dialog_state in ("new", "available_time") else models.model_4o_mini
        if backend == MCP_BACKEND_ALENA_5020:
            return models.model_4o_mini
        # classic
        return models.model_4o if dialog_state in ("postrecord",) else models.model_4o_mini
...
async def personalized_prompt(request: ModelRequest) -> str:
    """Формирование промпта из template/ файла (локальный путь)."""
    ...
    # В prod лучше логировать не весь prompt, а длину/хеш, но оставляю как было.
